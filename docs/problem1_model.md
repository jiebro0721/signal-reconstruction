...
TPR=100% 对盐椒噪声是结构性必然（真噪声像素取值恰为 $s_{\min}$/$s_{\max}$，
不可能通过严格不等式 $s_{\min}^{W}<y_{i,j}<s_{\max}^{W}$），报告它主要说明
AMF 判定与噪声模型一致而非经验检出能力；FPR 才是体现检测器质量的经验指标。
...
- **光滑参数 $\mu$**（该组为 $\beta=5,\alpha=100$、初值 AMF 输出的对照）：
  $\mu=1$ 时 169 步收敛（30%），$\mu=0.1$ 为 434 步，
  而 $\mu\le10^{-2}$ 时 2000 步后投影间隙仍 $\approx0.5$–0.8（不收敛）；
  三者的最终 PSNR 差 $<0.01$ dB（zcode 复验 100 步级差异为 0.0005 dB）；
- **求解器对照**（统一初值 AMF 输出, 每配置均为真实收敛结果）：

| 策略 | 迭代数 | 时间 (s) | PSNR (dB) | 收敛 |
|---|---|---|---|---|
| GPSR-Basic（沿投影弧 Armijo，maxit=10000） | 7160 | 344.11 | 37.373 | 是 |
| **GPSR-BB（BB1 + 非单调）** | **74** | **3.73** | **37.373** | 是 |
| GPSR-BB（BB2） | 76 | 3.89 | 37.373 | 是 |
| GPSR-BB + $\mu$ 续延 $(1,10^{-1},10^{-2},10^{-3})$ | 154 | 7.86 | 37.374 | 是 |
| 无数据项变体（文献 [3] 式 (9)） | 92 | 4.46 | 37.368 | 是 |

  （Lena 512 r=30%；另三个算例的对照见 `results/tables/problem1_strategy.txt`：
   Basic 迭代数为 5547–7180，BB1 为 74–88。）BB 步长把迭代数降低了
  **约 65–97 倍**（时间 69–92 倍），恢复质量完全一致；续延策略在小幅提升
  精度（+0.001 dB）的同时迭代数略增，故主实验直接用 $\mu=1$；
...
- **检测**：全部 72 个算例 TPR = 100%（盐椒噪声下为结构性必然，见 §5 口径说明），
  FPR 均值 2.7%–9.4%（30%）/ 1.2%–4.1%（50%），候选集大小
  $|\Omega|\approx8.3$–$9.6\times10^{4}$（30%）/$1.33$–$1.37\times10^{5}$（50%）；
- **迭代次数**：均值 87.3（30%，范围 38–190）、88.1（50%，范围 50–196）；
  72 个算例**全部**在终止准则下收敛；
- **第二阶段耗时**（空载环境）：均值 2.84 s（30%，范围 1.2–6.4 s）、
  4.59 s（50%，范围 2.6–10.8 s），随 $|\Omega|$ 近似线性；
- **PSNR**（12 图均值 36.38 / 33.56 dB，范围 30.1–47.8 dB（30%）、
  28.1–43.3 dB（50%））；信噪比 SNR 均值 30.94 / 28.12 dB，与 PSNR 差约
  5–13 dB（与图像对比度有关）；
...
4. 该方法在 12 张标准图上 30%/50% 椒盐噪声的 PSNR 均值分别为
   36.38 dB 与 33.56 dB（范围 30.1–47.8 / 28.1–43.3 dB；Lena 37.41 / 34.55 dB），
   与文献同配置结果一致或更优；
5. GPSR-BB 的 BB 步长是关键：相比 GPSR-Basic 迭代数降低约 65–97 倍
   （时间 69–92 倍），而恢复质量完全相同；
