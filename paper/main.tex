...
丢失，因此恢复工作分为``检测哪些像素被污染''与``恢复被污染像素''两个阶段\cite{chan2005}。
稀疏信号重构则是压缩感知的核心问题：利用远少于信号维数的线性观测恢复稀疏信号，
可归结为 $\ell_1$ 正则化最小二乘模型\cite{figueiredo2008}。
...
首先，通过变量分裂把模型化为有界约束二次规划\cite{figueiredo2008}后
...
全局收敛结论\cite{gilbert1992}，不能把非负截断本身等同于收敛保证。
...
\begin{tabular}{clc}
  ...
  符号 & 说明 & 单位 \\
  \midrule[1pt]
  $A$ & 图像像素索引集 $\{1,\ldots,M\}\times\{1,\ldots,N\}$（问题三中 $A$ 为测量矩阵，按所在问题区分） & -- \\
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
其中 $x_{i,j}$ 为原始像素值，$r=p+q$ 称为噪声等级。本文取
$p=q=r/2$。
为直观说明椒盐噪声的生成过程，以 Lena 灰度图像为例。对每个像素独立生成随机数
$\xi_{i,j}\sim U(0,1)$：当 $\xi_{i,j}<p$ 时，将像素置为 $s_{\min}=0$；当
$\xi_{i,j}\ge 1-q$ 时，将像素置为 $s_{\max}=255$；其余情况下保持原像素值不变。
...
结合像素端点取值与 AMF 的替换结果，定义噪声候选集为\cite{chan2005}：
\begin{equation}
...
\end{equation}
其中 $V_{i,j}=\{(i,j-1),(i,j+1),(i-1,j),(i+1,j)\}\cap A$ 为位置
$(i,j)$ 在图像域内的四邻域，
$\Omega^{C}$ 为 $\Omega$ 在 $A$ 中的补集。$S_1$ 表示候选像素与干净邻居之间的
...
式~\eqref{eq:smooth} 中的盒约束 $[s_{\min},s_{\max}]$ 引入是无损的，依据最大值原理\cite{cai2007}：当 $\varphi_{\alpha}$ 为偶函数、连续且关于
...
GPSR 方法\cite{figueiredo2008}原本用于稀疏重构：以变量分裂把
...
$\varphi_\alpha^{(k)}$，其余条件保持一致。五种候选势函数为\cite{cai2007}
\begin{align}
...
实验实例设置为\cite{figueiredo2008}：$n=4096$，$m=1024$，
...
$x_0=A^{\top}b$\cite{wu2021}。每段以相邻两步目标相对变化不超过 $10^{-6}$
...
GPSR 方法\cite{figueiredo2008}采用变量分裂：把 $x$ 的正部与负部分开，
...
去偏置过程\cite{figueiredo2008}：固定解的支撑集后做最小二乘
